...
// Add this function to pre-warm the backend
async function preWarmBackend() {
  try {
    ...
    // Don't show error to user, just let it wake up naturally
  }
}
...
async function onClickedEstimatePrice() {
  ...
  try {
    ...
    estPrice.innerHTML = `<h2 class='text-red-500 text-center font-bold'>Backend is waking up. Please try again in 30 seconds.</h2>`;

    // Auto-retry after 5 seconds
    setTimeout(() => {
      estPrice.innerHTML = `<h2 class='text-yellow-500 text-center font-bold'>Ready to try again</h2>`;
    }, 5000);
  }
}
...
async function loadLocations() {
  ...
  // Show loading state
  uiLocations.innerHTML = `<option value="" disabled selected>Loading locations...</option>`;
  ...
  try {
    ...
    uiLocations.innerHTML = `<option value="" disabled selected>Failed to load locations. Please refresh.</option>`;

    // Retry after 3 seconds
    setTimeout(loadLocations, 3000);
  }
}
